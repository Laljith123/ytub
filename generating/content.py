...
BANNED_SCRIPT_PHRASES = tuple(
    ...
    for phrase in os.getenv(
        "CONTENT_BANNED_SCRIPT_PHRASES",
        "lowkey|no cap|true crime fam|let's dive in|lets dive in|breaking news|"
        "this just in|welcome back|smash like|like and subscribe|i still remember|"
        "headlines screamed|soft gasp|quick breath|takes a breath|hold up|yo",
    ).split("|")
    if phrase.strip()
)
REQUIRED_FIELDS = {
...
def _contains_banned_script_phrase(items: list) -> bool:
    raw = " ".join(str(item or "") for item in items).lower()
    ...
    for phrase in BANNED_SCRIPT_PHRASES:
        ...
        if phrase_norm and len(phrase_norm.split()) > 1 and phrase in raw:
            return True
        if not phrase_norm:
            continue
        if len(phrase_norm.split()) == 1:
            if phrase_norm in normalized_tokens:
                return True
...
def _final_scene_invites_comments(script: list) -> bool:
    if not script:
        return False
    final = str(script[-1] or "").strip()
    final_norm = _normalize(final)
    tokens = set(final_norm.split())
    has_question = "?" in final
    has_comment_hook = bool(
        tokens
        & {
            "comment",
            "comments",
...
def _trend_input_text(item) -> str:
    if isinstance(item, dict):
        for key in ("case_name", "trend", "title", "source_title", "keyword"):
...
def _trend_source_context(items: list) -> list[dict]:
    context: list[dict] = []
    seen: set[str] = set()
    for item in items:
        if not isinstance(item, dict):
            continue
        case_name = _strip_trend_text(_trend_input_text(item))
        norm = _normalize(case_name)
        if not case_name or not norm or norm in seen:
            continue
        seen.add(norm)
        compact = {
            "case_name": case_name,
        ...
        context.append({key: value for key, value in compact.items() if value})
    return context


def _clean_trend_list(items: list, *, allow_numeric: bool = True) -> list[str]:
    cleaned: list[str] = []
    seen: set[str] = set()
    for item in items:
        if item is None:
            continue
        value = _strip_trend_text(_trend_input_text(item))
        if not value:
            continue
...
def _load_json_list(path: Path, label: str) -> list[dict]:
    ...
    try:
        ...
        print(f"{label} skipped: {path} is not valid JSON.")
        return []
    except OSError as exc:
        print(f"{label} skipped: unable to read {path}: {exc}")
        return []
...
def _description_memory(text: object) -> str:
    value = str(text or "").strip()
    if not value:
        return ""
    value = re.split(r"\n\s*\n\s*#", value, maxsplit=1)[0]
    value = re.split(
        r"\n\s*\n\s*(?:shorts|ytshorts|youtubeshorts|true crime|crime)\s*,",
        value,
        maxsplit=1,
        flags=re.IGNORECASE,
    )[0]
...
def _history_record_to_content_item(item: dict) -> dict:
    ...
    if isinstance(script, list):
        script_memory = [str(part) for part in script if str(part).strip()]
    elif script:
        script_memory = [str(script)]
    else:
        description = _description_memory(item.get("description"))
        script_memory = [description] if description else []

    image = item.get("image")
    if isinstance(image, list):
        image_memory = [str(part) for part in image if str(part).strip()]
    elif image:
        image_memory = [str(image)]
    else:
        image_memory = []
...
def _load_generation_history() -> list[dict]:
    records = [_history_record_to_content_item(item) for item in _load_json_list(HISTORY_JSON, "History")]
    records = [record for record in records if record.get("title") or record.get("trend")]
    if records:
        print(f"Loaded {len(records)} upload history records from {HISTORY_JSON}.")
    return records
...
def _build_prompt(trends: list[str], repeated: list[str], channel_titles: list[str]) -> str:
    ...
    source_context = _trend_source_context(trends)
    source_context_line = (
...
def _run_completion(
    ...
    extra_body = json_extra_body(CONTENT_BASE_URL, enable_thinking, REASONING_BUDGET)
    if extra_body:
...
def contents(trends):
    ...
    for item in comparison_data:
        for key in ("trend", "title"):
            value = str(item.get(key) or "").strip()
            norm = _normalize(value)
            if value and norm and norm not in seen_repeated:
