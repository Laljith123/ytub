...
def _run_thumbnail_completion(prompt: str) -> str:
    ...
    extra_body = json_extra_body(
        THUMBNAIL_PLAN_BASE_URL,
        THUMBNAIL_PLAN_ENABLE_THINKING,
        THUMBNAIL_PLAN_REASONING_BUDGET,
    )
    if extra_body:
...
def _build_thumbnail_plan(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if not THUMBNAIL_PLAN_API_KEY:
        print("Thumbnail planner skipped: no JSON prompt API key found. Using title fallback.")
        return _validate_thumbnail_plan(default_plan, item)
    ...
    for attempt in range(1, max(1, THUMBNAIL_PLAN_MAX_ATTEMPTS) + 1):
        try:
            print(
                f"[Thumbnail] Planning attempt {attempt}/{THUMBNAIL_PLAN_MAX_ATTEMPTS} "
                f"using {json_provider_name(THUMBNAIL_PLAN_BASE_URL)} model {THUMBNAIL_PLAN_MODEL}..."
            )
