...
def _load_json_list(path: Path, label: str) -> list[dict]:
    ...
    try:
        ...
        print(f"Trend collision check skipped: {label} {path} is not valid JSON.")
        return []
    except OSError as exc:
        print(f"Trend collision check skipped: unable to read {label} {path}: {exc}")
        return []
...
def _description_topic_text(text: object) -> str:
...
def _load_used_topics() -> list[str]:
    """Read current output plus durable history so deleted temp files do not erase memory."""
    data = _load_json_list(OUTPUT_JSON, "output") + _load_json_list(HISTORY_JSON, "history")
    used: list[str] = []
    seen: set[str] = set()
    for item in data:
        candidates = [
            ...
            item.get("title"),
            _description_topic_text(item.get("description")),
        ]
        for candidate in candidates:
            value = _content_trend_text(str(candidate or ""))
...
def _filter_used_candidates(candidates: list[dict], used_topics: list[str]) -> list[dict]:
    ...
    if removed:
        print(f"Filtered {removed} candidates already used in output/history.")
    return filtered
...
def main() -> None:
    ...
    if used_topics:
        print(f"Loaded {len(used_topics)} used trends/titles from output/history for collision checks.")
        ...
        content_trends.append(
            {
                ...
                "source_title": str(item.get("source_title") or "").strip(),
                "source": source,
                "shorts_score": int(score or 0),
                "safety": str(item.get("safety") or "").strip(),
            }
        )

    contents(content_trends or trend_titles)
