...
VOICE_SAMPLE_PATH = Path(os.getenv("VOICE_SAMPLE_PATH", str(VOICE_SAMPLE)))
VOICE_SAMPLE_B64 = os.getenv("VOICE_SAMPLE_B64", "")
...
def _resolve_voice_sample() -> Path:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    target = OUTPUT_DIR / "voice_sample.wav"

    if VOICE_SAMPLE_B64:
        data = VOICE_SAMPLE_B64
        if data.startswith("data:"):
            data = data.split(",", 1)[-1]
        try:
            audio_bytes = base64.b64decode(data)
        except Exception as exc:
            raise RuntimeError("VOICE_SAMPLE_B64 is not valid base64.") from exc
        target.write_bytes(audio_bytes)
        return target

    if VOICE_SAMPLE_URL:
        urlretrieve(VOICE_SAMPLE_URL, target)
        return target
    ...
    raise RuntimeError(
        "Voice sample not found. Provide voices/master.wav, or set VOICE_SAMPLE_PATH, "
        "VOICE_SAMPLE_B64, or VOICE_SAMPLE_URL."
    )
...
paths = []
voice_sample = _resolve_voice_sample()
