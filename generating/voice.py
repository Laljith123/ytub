...
def _build_voice_plan_prompt(video_data: dict, chunks: list[str]) -> str:
    ...
    return (
        ...
        "Use alert energy for hooks, calmer pacing for facts, slower pacing for reveals, "
        "small pauses after suspicious details, and a longer pause after the final comment question. "
...
def build_voice_plan(video_data: dict, chunks: list[str]) -> list[dict]:
    ...
    print(f"Voice plan using {json_provider_name(VOICE_PLAN_BASE_URL)} model {VOICE_PLAN_MODEL}.")
