...
UPLOAD_HISTORY_FILE = Path(
    os.getenv("UPLOAD_HISTORY_FILE", str(ROOT / "generating" / "history.json"))
)
SAVE_UPLOAD_HISTORY = os.getenv("SAVE_UPLOAD_HISTORY", "1") == "1"
...
MUSIC_ATTRIBUTION_FILE = Path(
    os.getenv("MUSIC_ATTRIBUTION_FILE", str(OUTPUT_DIR / "music" / "background_attribution.txt"))
)
...
def _load_upload_history() -> list[dict]:
    ...
    try:
        ...
        print(f"Upload history ignored: {UPLOAD_HISTORY_FILE} is not valid JSON.")
        return []
    except OSError as exc:
        print(f"Upload history ignored: unable to read {UPLOAD_HISTORY_FILE}: {exc}")
        return []
    return data if isinstance(data, list) else []


def _append_upload_history(
    *,
    video_id: str,
    title: str,
    description: str,
    ...
    if not SAVE_UPLOAD_HISTORY or not video_id:
        return

    history = _load_upload_history()
    if any(isinstance(item, dict) and item.get("id") == video_id for item in history):
        return
    ...
    for key in ("trend", "hook", "caption", "thumbnail_text", "background_music"):
        value = latest_content.get(key) if isinstance(latest_content, dict) else None
        if value:
            record[key] = value
    ...
    try:
        ...
        print(f"Upload history updated: {UPLOAD_HISTORY_FILE}")
    except OSError as exc:
        print(f"Unable to update upload history {UPLOAD_HISTORY_FILE}: {exc}")


def _music_attribution_text() -> str:
    if not MUSIC_ATTRIBUTION_FILE.exists():
        return ""
    try:
        return MUSIC_ATTRIBUTION_FILE.read_text(encoding="utf-8").strip()
    except OSError:
        return ""


def _append_music_attribution(description: str) -> str:
    attribution = _music_attribution_text()
    if not attribution or attribution in description:
        return description
...
def generate_metadata_plan(latest: dict, fallback_title: str, fallback_description: str) -> dict:
    ...
    for attempt in range(1, METADATA_MAX_ATTEMPTS + 1):
        print(
            f"\n[Metadata] Attempt {attempt}/{METADATA_MAX_ATTEMPTS} using "
            f"{json_provider_name(METADATA_BASE_URL)} model {METADATA_MODEL} ..."
        )
        try:
            request = {
...
def _load_defaults() -> Tuple[str, str]:
    ...
    if REFERENCE_TEXT:
        description = f"{description}\n\n{REFERENCE_TEXT}"
    description = _append_music_attribution(description)
...
def main() -> None:
    ...
    base_description = _append_music_attribution(base_description)
